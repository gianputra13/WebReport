    ...
    <div class="card shadow mb-4">
      <div class="card-header py-3 d-flex justify-content-between">
        <form action="" class="form-inline">
          ...
          <button
            type="button"
            class="btn btn-primary btn-sm ml-md-3 w-xs-100 shadow-sm"
        ...
        <a
          ...
          @click="requestEmailTrasaction()"
          ><i class="fas fa-file-excel fa-sm text-white-50"></i> Generate
          Excel</a
        >
      </div>
  ...
  methods: {
    requestEmailTrasaction() {
      ...
      if (a.diff(b, "days") >= 7) {
        ...
        //     showLoaderOnConfirm: true,
        //     preConfirm: () => {
        //         return axios.post('/api/emailUser', params, { validateStatus: false })
        //         .then(response => {
        //             if(response.status==200){
        //                 Swal.insertQueueStep({
        //                     icon: 'success',
        //                     title: 'Sukses',
        //                     text: 'Email telah dikirim ke '+response.data.message+' (cek inbox & spam)'
        //                 })
        //             } else {
        //                 Swal.insertQueueStep({
        //                     icon: 'error',
    ...
    async getHistoryTransaction() {
      ...
      if (a.diff(b, "days") >= 7) {
        ...
        this.isLoading = true;
        let respon = await this.getTransaksiHistoryNonPajak(
          params
        );
        // console.log(respon);
